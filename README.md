...
><i>잠깐!</i> 이 단계에서 `SaaS app (with DB and user auth)`를 선택해도 되지만, 그럴 경우 API의 경로가 달라지므로 우선은 Rest API를 선택하고 나중에 Loco에 더 알게 되면 SaaS app으로도 진행해 보세요.
```
...
* [Visual Studio Code](https://code.visualstudio.com/) - VSCode 설치 프로그램 다운로드하여 실행
* [VSCode와 wsl 연동](https://learn.microsoft.com/ko-kr/windows/wsl/tutorials/wsl-vscode)
* VSCode에 [Rust Analyzer](https://marketplace.visualstudio.com/items?itemName=rust-lang.rust-analyzer) 설치
